  ...
  for (let j = 0; j < 7; j++) {
    ...
    for (let k = 0; k < 10; k++) {
      ...
      for (let i = 0; i < timetable["data"].length; i++) {
        // all timetables
        if (
          timetable["data"][i]["timetable"][j][k].roomid != "" ||
          timetable["data"][i]["timetable"][j][k].teacherid != ""
        ) {
          continue;
        } else if (
          timetable["data"][i]["timetable"][j][k].teacherid &&
          ...
          if (
            ...
            process.stdout.write(
              temp["room" + timetable["data"][i]["timetable"][j][k].roomid]
                ? "room conflicts "
                : "room error safe",
            );
            process.stdout.write("    --------    ");
            process.stdout.write(
              temp[
                "teacher" + timetable["data"][i]["timetable"][j][k].teacherid
              ]
                ? "teacher conflicts "
                : "teacher error safe",
            );
